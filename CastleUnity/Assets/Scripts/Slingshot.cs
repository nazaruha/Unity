...
{
    static private Slingshot S; // скритий статичний екземпляр Slingshot
    ...
    static public Vector3 LAUNCH_POS // початкове положення об'єкту
    {
        get
        {
            if (S == null) return Vector3.zero; // якщо поле S == null, то поверне [0, 0, 0]
            return S.launchPos;
        }
    }

    private void Awake() // запускається ще перед запуском сцени. это самый первый метод, который вызывается после создания экземпляра любого класса, наследующего MonoBehaviour, поле S будет установлено до того, как какой-то другой код попробует обратиться к свойству LAUNCH_POS.
    {
        S = this; //Здесь экземпляр this класса Slingshot присваивается полю S
